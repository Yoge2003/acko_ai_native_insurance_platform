"""
Claims Explainer.
Generates local, global, and human-readable explanations using SHAP for Car and Bike Claims models.
"""
...
class ClaimsExplainer(BaseExplainer):
    """
    Explainer specialized for binary classification models (Car and Bike Claims).
    """
    ...
    def explain_prediction(self, X_input: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        logger.info("Computing local SHAP explanations for claims prediction.")
        ...
        prediction_val = float(self.model.predict_proba(X_row)[0, 1])
        ...
        if pos_terms:
            if len(pos_terms) > 1:
                ...
                pos_str = pos_terms[0]
            text += f"The claim approval probability increased because the policy has acceptable parameters in {pos_str}."
        ...
        if neg_terms:
            ...
            if text:
                text += " "
            text += f"The claim approval probability decreased because potential risks were flags in {neg_str}."

        if not text:
            text = "The claim approval expectation lies close to the baseline approval probability."
